...
public static class MonkeyHelper
{
    ...
    /// <summary>
    /// Initializes the monkey collection with sample monkey data from various regions.
    /// </summary>
    public static void InitializeMonkeys()
    {
        if (MonkeyCollection.Count > 0)
            return;

        MonkeyCollection.AddRange(new[]
        {
            new Monkey
            {
                Name = "Baboon",
    ...
    {
        return MonkeyCollection.FirstOrDefault(m => 
    ...
    /// </summary>
    /// <returns>A random monkey, or null if the collection is empty.</returns>
    public static Monkey? GetRandomMonkey()
    {
        if (MonkeyCollection.Count == 0)
            return null;

        var randomIndex = Random.Next(MonkeyCollection.Count);
        return MonkeyCollection[randomIndex];
    }

    /// <summary>
    /// Gets the count of monkeys in the collection.
    /// </summary>
    /// <returns>The number of monkeys available.</returns>
    public static int GetMonkeyCount()
    {
        return MonkeyCollection.Count;
    }
}
